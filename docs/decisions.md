...
## 2026-04-09 — Phase 6: UX 개편 — 탐색형 → 권고형 전환

**결정:** 제품의 성격을 "사용자가 모든 변수를 이해한 뒤 조작하는 탐색형"에서 "제품이 먼저 중요한 레버를 제시하고 사용자는 확인·조정하는 권고형"으로 전환한다.

**이유:**
- 30+ 슬라이더가 동등 가중치로 나열되면 "어디부터 만질지"에 답이 없음.
- 민감도 결과가 전용 탭 안에만 있으면, 입력(슬라이더)과 결과(우선순위)가 분리됨.
- 매장 매니저 단독 + 팀 워크숍 둘 다 "합의 속도"가 핵심. 기능 수보다 리듬 설계가 중요.

**구체 결정들:**

**1. 민감도를 입력 패널 전반의 시각 위계 데이터로 사용**
- App에서 `useDeferredValue` + `useMemo`로 sensitivity 계산, 원본 전파 금지.
- 축약 leverMap `{ rank, isTopLever, upPct }`만 하향 전달.
- Top 5 슬라이더 강조, 나머지 dim(opacity 0.55).

**2. 탭 위계화 — 콘텐츠 분류가 아닌 의사결정 단계 반영**
- 주 탭: 민감도(무엇을) / 목표 역산(얼마나) / 일별 캘린더(언제).
- 보조 탭: 주간 / 전환퍼널 / CRM퍼널 / 평일vs주말 (심화/참고).
- 기본 탭을 민감도로 변경.

**3. KPI tone 카테고리 — KPI별 고정 룰 대신 의미 기준**
- goal-tracked (월매출, 목표달성률): 목표 대비율 기준.
- growth (회원, 재구매): accent 고정.
- neutral (객단가, 유입 등): default 고정, delta badge로만 표현.

**4. 인사이트 구조 확장 — message-only → reason+action+targetControl**
- `targetControl: "domain.field"` 추가. 클릭 시 슬라이더로 스크롤 + 1.2초 soft glow.
- 단순 jump가 아닌 suggestedDelta까지 표시.

**의도적으로 하지 않은 것:**
- 모바일 반응형 (데스크탑 전용 유지).
- named scenario save (별도 큰 작업, 이번 범위 외).
- undo/redo, 코치마크 (A1 — 구조가 스스로 설명되면 의존도 낮아짐).
